...
def get_user_templates(
    *,
    ...
    templates = get_all_template_names(cursor)
...
def add_new_model(
    *,
    ...
    sql_file_name = get_template_file_name(
        cursor,
        model_template,
    ...
    *,
    ...
    for source_project, model_names in models_by_project.items():
        ...
        for model_name in model_names:
            ...
            updated = move_model_to_project2(
                cursor,
                owner_email,
                model_name,
                source_project,
...
def get_user_models_by_project(
    *,
    ...
    rows = get_models_by_user_grouped(
        cursor,
        user_email
    )
...
def save_as_model(
    *,
    ...
    is_user_email = valid_email(save_as_from_user_email)

    # 1. Resolve existing model + path
    model_id, model_path = get_model_id_and_path(
        cursor,
        existing_name,
        project_name,
        #owner_email
...
def rename_model(
    *,
    ...
    new_model_id, new_model_path = get_model_id_and_path(
        cursor,
        new_model_name,
        project_name,
        owner_email
...
def backup_model(
    *,
    ...
    count = get_backup_count_by_model(cursor, model_id) #added
...
def accept_model_share(
    *,
    ...
    project_id = get_project_id(cursor, owner_email, current_project)

    # add refference to s_usermodels
    updated = accept_model(
        cursor,
        new_model_id,
        project_id,
        owner_email,
    ...
    row = share_model_request_accepted(
        cursor,
        notification_id,
        owner_email,
        "1",
...
def move_model_to_project2(cursor, user_email: str, model_name: str, old_project_name: str, new_project_name: str) -> int:
    old_Model_id, old_Model_path = get_model_id_and_path(cursor, model_name, old_project_name, user_email)
    if not old_Model_id:
        return 0
    new_Model_id, new_Model_path = get_model_id_and_path(cursor, model_name, new_project_name, user_email)
    if new_Model_id:
        return 0
...
def add_user_model(
    ...
    _Model_id, _Model_path = get_model_id_and_path(cursor, model_name, project_name, user_name)
    if _Model_id:
